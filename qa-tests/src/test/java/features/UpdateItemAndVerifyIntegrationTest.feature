Feature: Update Item flow with Kafka and DB verification
  ...
  Scenario: User updates an item and system propagates changes via Kafka
    ...
    * def updateEvent = call read('classpath:karatehelpers/kafka-wait.feature') { itemId: '#(itemId)', eventType: 'ITEM_UPDATED', timeout: 15000 }
    * print 'Received Kafka update event:', updateEvent  
    Then match updateEvent.message.eventType == 'ITEM_UPDATED'
    And match updateEvent.message.payload.id == itemId
    And match updateEvent.message.payload.name == 'Monitor-HD'
    And match updateEvent.message.payload.quantity == 10
